...
export const initProductCarousel = () => {
  ...
  let slidesArray = Array.from(slides); // Конвертируем NodeList в массив для работы
  ...
  const moveToNextSlide = () => {
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < slidesPerView; i++) {
        const slide = slidesArray[(currentIndex + i) % slidesArray.length];
      ...
      // Обновляем индекс
      currentIndex = (currentIndex + slidesPerView) % slidesArray.length;
      
      // Обновляем коллекцию слайдов
      const newSlides = document.querySelectorAll('.slide');
      slidesArray = Array.from(newSlides); // Обновляем массив слайдов
    }, 500);
  };
